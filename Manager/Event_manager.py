...
class EventManager:
    def __init__(self):
        ...
        dispatcher.connect(self.handle_update_enemy_action, signal=Signals.Enemy_Update, sender=dispatcher.Any)
        dispatcher.connect(self.handle_tower_upgrade, signal=Signals.Tower_Upgrade,
    ...
    def handle_update_enemy_action(self, sender, event):
        self.enemy_action(event,Action.Enemy_Update)

    def handle_enemy_remove_action(self, sender, event):
        self.enemy_action(event,Action.Remove_Enemy)

    def handle_tower_upgrade(self, sender, event):
        self.handel_action(sender, event, Action.Tower_Attack_Upgrade)
